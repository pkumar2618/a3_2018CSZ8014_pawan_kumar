...
test_X = train_XY.loc[:,:'C5_13'].to_numpy(copy=True)
test_Y = train_XY.loc[:,'Hand_0':'Hand_9'].to_numpy(copy=True)

### Reading Configuration file ###
config ={'neu_input_layer': 1, 'neu_output_layer': 1, 'batch_size': 100, 'n_hidden_layers': 2, 'neu_in_layers':[85,5,10],
         'non_linearity': 'sigmoid', 'learning_rate': 0.1 }
...
if question_part == 'c':
    ...
    for i in [5, 10, 15, 20, 25]:
        ...
        parameters = L_layer_model(train_X.T, train_Y.T, batch=config['batch_size'],
                                   ...
                                   learning_rate=config['learning_rate'], num_iterations=800, print_cost=True)
        end = time.time()
        ...
        acc_te, p_te = predict(train_X.T, train_Y.T, parameters)
